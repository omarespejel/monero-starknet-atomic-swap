...
impl SharedOutput {
    ...
    pub fn from_public(alice: &AlicePublicData, bob: &BobPublicData) -> Result<Self> {
        let S_a = CompressedEdwardsY(alice.S_a).decompress()
            .ok_or_else(|| anyhow::anyhow!("Invalid S_a"))?;
        let V_a = CompressedEdwardsY(alice.V_a).decompress()
            .ok_or_else(|| anyhow::anyhow!("Invalid V_a"))?;
        let S_b = CompressedEdwardsY(bob.S_b).decompress()
            .ok_or_else(|| anyhow::anyhow!("Invalid S_b"))?;
        let V_b = CompressedEdwardsY(bob.V_b).decompress()
