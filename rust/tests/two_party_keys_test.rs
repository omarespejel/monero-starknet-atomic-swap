...
fn test_combined_spend_key() {
    ...
    assert_eq!(
        shared.S, expected_s,
        "Combined spend key must equal sum of shares"
    );
}
...
fn test_combined_view_key() {
    ...
    assert_eq!(
        shared.V, expected_v,
        "Combined view key must equal sum of shares"
    );
}
...
fn test_security_recovery_requires_both() {
    ...
    let wrong = recover_spend_key(
        alice.spend_share(),
        curve25519_dalek::scalar::Scalar::from(99999u64),
    );
...
fn test_alice_public_data_validation() {
    ...
    assert!(
        ...
        "Valid AlicePublicData should pass validation"
    );
...
fn test_bob_public_data_validation() {
    ...
    assert!(
        zero_hashlock.validate().is_err(),
        "Zero hashlock should fail validation"
    );
}
...
fn test_alice_zero_scalar_rejection() {
    ...
    for _ in 0..1000 {
        ...
        assert_ne!(
            ...
            "Alice's spend share must never be zero"
        );
        assert_ne!(
            alice.view_share(),
            curve25519_dalek::scalar::Scalar::ZERO,
            "Alice's view share must never be zero"
        );
    }
}
...
fn test_bob_zero_scalar_rejection() {
    ...
    for _ in 0..1000 {
        ...
        assert_ne!(
            bob.spend_share(),
            curve25519_dalek::scalar::Scalar::ZERO,
            "Bob's spend share must never be zero"
        );
...
fn test_malicious_alice_attack_prevention() {
    ...
    assert_ne!(
        legitimate_shared.S, fake_shared.S,
        "Fake S_a must produce different address"
    );
    ...
    assert_eq!(
        legitimate_recovered * G,
        legitimate_shared.S,
        "Legitimate recovery works"
    );
    assert_ne!(
        fake_recovered * G,
        legitimate_shared.S,
        "Fake S_a cannot steal funds"
    );
    assert_eq!(
        fake_recovered * G,
        fake_shared.S,
        "Fake recovery matches fake address"
    );
}
...
fn test_security_secret_reuse_attack() {
    ...
    assert_ne!(
        shared1.S, shared2_attempt.S,
        "Secret reuse must produce different address"
    );
    ...
    assert_ne!(
        recovered2_attempt * G,
        shared1.S,
        "Cannot steal from swap 1 with swap 2 keys"
    );
    assert_eq!(
        recovered2_attempt * G,
        shared2_attempt.S,
        "Swap 2 recovery works"
    );
}
