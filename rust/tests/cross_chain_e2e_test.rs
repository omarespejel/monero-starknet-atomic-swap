...
async fn test_rust_to_cairo_dleq_roundtrip() -> Result<()> {
    ...
    println!("   📍 Shared spend key point: {:?}", hex::encode(shared.S.compress().to_bytes()));
    ...
    println!("   📍 Challenge: {:?}", hex::encode(dleq_proof.challenge.to_bytes()));
    println!("   📍 Response: {:?}", hex::encode(dleq_proof.response.to_bytes()));
    ...
    println!("   📍 Second point compressed: {:?}", hex::encode(cairo_format.second_point_compressed));
    ...
    assert_eq!(cairo_format.adaptor_point_compressed.len(), 32, "Adaptor point must be 32 bytes");
    assert_eq!(cairo_format.second_point_compressed.len(), 32, "Second point must be 32 bytes");
    ...
    assert_eq!(cairo_format.challenge.len(), 32, "Challenge must be 32 bytes");
