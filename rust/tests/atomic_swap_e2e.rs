...
fn test_full_atomic_swap_flow() {
    ...
    assert!(
        dleq_proof.challenge.to_bytes() != [0u8; 32],
        "Challenge must be non-zero"
    );
    assert!(
        dleq_proof.response.to_bytes() != [0u8; 32],
    ...
    let recovered_full_key = SwapKeyPair::recover(partial_key_zeroizing, revealed_secret);

    // Verify recovery is correct
    assert_eq!(
        *recovered_full_key, alice_keys.full_spend_key,
...
fn test_swap_fails_with_wrong_secret() {
    ...
    // Wrap partial_key in Zeroizing for memory safety
    let partial_key_zeroizing = Zeroizing::new(alice_keys.partial_key);
    let wrong_recovered = SwapKeyPair::recover(partial_key_zeroizing, wrong_secret);
    assert_ne!(
        *wrong_recovered, alice_keys.full_spend_key,
...
fn test_multiple_swaps_independent() {
    ...
    let hashlock1: [u8; 32] = Sha256::digest(alice1_keys.adaptor_scalar_bytes()).into();
    let hashlock2: [u8; 32] = Sha256::digest(alice2_keys.adaptor_scalar_bytes()).into();
    assert_ne!(
        hashlock1, hashlock2,
        "Different swaps must have different hashlocks"
    );
