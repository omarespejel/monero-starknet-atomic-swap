...
use xmr_secret_gen::dleq::ed25519_bn254::{
    ed25519_scalar_to_bn254_safe,
    Ed25519Bn254ConversionError,
    ED25519_ORDER,
    BN254_PRIME,
};
...
fn test_conversion_preserves_small_values() {
    ...
    let converted = ed25519_scalar_to_bn254_safe(&scalar)
        .expect("Small scalar must convert");
...
fn test_canonical_vector_conversion() {
    ...
    let converted = ed25519_scalar_to_bn254_safe(&scalar)
        .expect("Canonical vector must convert");
...
fn test_zero_scalar_converts() {
    ...
    let converted = ed25519_scalar_to_bn254_safe(&zero)
        .expect("Zero scalar must convert");
...
fn test_one_scalar_converts() {
    ...
    let converted = ed25519_scalar_to_bn254_safe(&one)
        .expect("One scalar must convert");
