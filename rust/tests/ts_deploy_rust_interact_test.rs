...
async fn deploy_via_typescript() -> Result<String> {
    ...
    let deployment_json: Value = serde_json::from_str(
        &fs::read_to_string(&deployment_file).context("Failed to read deployment result file")?,
    )?;
...
async fn test_ts_deploy_rust_interact() -> Result<()> {
    ...
    println!("\n📝 Step 2: Generate DLEQ Proof");
    let _dleq_proof = generate_dleq_proof_for_bob(&bob).context("Failed to generate DLEQ proof")?;
    ...
    let client_manual =
        StarknetManualClient::devnet(DEVNET_ACCOUNT, DEVNET_PRIVATE_KEY, class_hash)?;
    ...
    let test_vectors_json: Value = serde_json::from_str(
        &fs::read_to_string(&test_vectors_path).context("Failed to read test vectors")?,
    )?;
    ...
    let view_client = StarknetManualClient::devnet(DEVNET_ACCOUNT, DEVNET_PRIVATE_KEY, "0x0")?;
    ...
    assert!(
        !is_revealed_before,
        "Secret should not be revealed initially"
    );
    assert!(
        !is_unlocked_before,
        "Contract should not be unlocked initially"
    );
    ...
    let reveal_tx_hash = client
        .reveal_secret(&contract_address, &secret_bytes_array)
        .await
    ...
    let receipt = view_client
        .wait_for_transaction(&reveal_tx_hash)
        .await
    ...
    println!(
        "   📍 is_secret_revealed: {} (expected: true)",
        is_revealed_after
    );
    println!(
        "   📍 is_unlocked: {} (expected: false, grace period active)",
        is_unlocked_after
    );

    assert!(
        is_revealed_after,
        "Secret should be revealed after reveal_secret()"
    );
...
async fn test_full_swap_flow_hybrid() -> Result<()> {
    ...
    let client: Box<dyn StarknetClient> = Box::new(StarknetManualClient::devnet(
        DEVNET_ACCOUNT,
        DEVNET_PRIVATE_KEY,
    ...
    let reveal_tx_hash = client
        .reveal_secret(&contract_address, &secret_bytes)
        .await?;
    println!("   ✅ Secret revealed!");
    println!("   📍 Transaction hash: {}", reveal_tx_hash);
    ...
    println!(
        "   ✅ is_unlocked: {} (expected: false, grace period)",
        is_unlocked
    );
